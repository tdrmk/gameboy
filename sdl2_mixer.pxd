...
cdef extern from "SDL2/SDL_mixer.h" nogil:
    ...
    int MIX_CHANNELS # macro
    int MIX_DEFAULT_FREQUENCY # macro
    int MIX_DEFAULT_FORMAT # macro
    int MIX_DEFAULT_CHANNELS # macro
    int MIX_MAX_VOLUME # macro
    ...
    Mix_Chunk* Mix_LoadWAV(const char* file) # macro
    ...
    int Mix_GroupChannels(int frm, int to, int tag)
    int Mix_GroupAvailable(int tag)
    int Mix_GroupCount(int tag)
    int Mix_GroupOldest(int tag)
    int Mix_GroupNewer(int tag)
    int Mix_PlayChannel(int channel, Mix_Chunk* chunk, int loops) # macro
    int Mix_PlayChannelTimed(int channel, Mix_Chunk* chunk, int loops, int ticks)
    int Mix_PlayMusic(Mix_Music* music, int loops)
    int Mix_FadeInMusic(Mix_Music* music, int loops, int ms)
    int Mix_FadeInMusicPos(Mix_Music* music, int loops, int ms, double position)
    int Mix_FadeInChannel(int channel, Mix_Chunk* chunk, int loops, int ms) # macro
